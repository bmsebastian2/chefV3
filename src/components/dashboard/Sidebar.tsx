      ...
      {open && (
        <div className="absolute bottom-full left-0 right-0 mx-4 mb-2 bg-white border border-zinc-200 rounded-xl shadow-lg overflow-hidden z-50">
          ...
          <div className="py-1">
            ...
            <form action={logout}>
              <LogoutButton className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-red-600 hover:bg-red-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors" />
            </form>
          </div>
        </div>
      )}
...
export function Sidebar({
  ...
  return (
    <>
      ...
      {/* Mobile drawer */}
      <div
        className={`fixed top-0 right-0 z-50 h-full w-64 bg-white shadow-xl md:hidden transition-transform duration-300 ${
