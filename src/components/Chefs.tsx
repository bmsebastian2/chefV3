...
// Card compacta: foto + nombre + especialidad + rating. Sin el subrayado de
// acento, años de experiencia ni footer "Ver perfil" de la card original —
// esos elementos se sacaron para bajar la altura y que entren más chefs de un
// vistazo (grid denso en home y en /chefs). Reutiliza ChefPhoto y el mismo
// bloque de rating; la lógica de rating no cambia.
export function ChefCardCompact({
  chef,
  onSelect,
}: {
  chef: ChefCard;
  onSelect: (chef: ChefCard) => void;
}) {
  ...
  return (
    ...
    >
      {/* Retrato — B&N en reposo, revela color al hover/focus */}
      <div className="relative aspect-[4/5] overflow-hidden rounded-lg ring-1 ring-black/5 transition-shadow duration-300 group-hover:shadow-xl group-hover:shadow-zinc-900/[0.10] group-focus-visible:ring-2 group-focus-visible:ring-accent">
        <ChefPhoto
          src={chef.imageUrl}
          name={chef.name}
          sizes="(max-width: 768px) 100vw, (max-width: 1024px) 33vw, 25vw"
          imgClassName="grayscale transition-[transform,filter] duration-700 ease-out group-hover:scale-105 group-hover:grayscale-0 group-focus-visible:grayscale-0 motion-reduce:transition-none"
        />
        <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-black/55 via-black/10 to-transparent" />
        {location && (
          <span className="absolute inset-x-4 bottom-4 text-[10px] font-semibold uppercase tracking-[0.24em] text-white/90">
            {location}
          </span>
        )}
        {chef.isPro && (
          <span className="absolute right-4 top-4 rounded-full bg-accent px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.18em] text-accent-foreground shadow-sm">
            Pro
          </span>
        )}
      </div>
      ...
      <div className="px-1 pt-3">
        <h3 className="font-serif text-lg font-semibold leading-tight text-zinc-900">
          {chef.name}
        </h3>
        {/* min-h reservado: si no hay tagline, la fila no colapsa y el grid no queda dentado */}
        <p className="mt-1 line-clamp-1 min-h-[1.1rem] font-sans text-sm font-light text-zinc-500">
          {chef.tagline ?? " "}
        </p>
        <div className="mt-2 flex items-center gap-2">
          {chef.rating.count > 0 ? (
            <>
              <span className="font-serif text-base italic text-zinc-900">
                {chef.rating.average.toFixed(1)}
              </span>
              <span className="h-1 w-1 rounded-full bg-accent" aria-hidden="true" />
              <span className="text-xs font-medium text-zinc-500">
                {chef.rating.count} {chef.rating.count === 1 ? "reseña" : "reseñas"}
              </span>
            </>
          ) : (
            <span className="inline-flex w-fit items-center rounded-full border border-zinc-200 px-2.5 py-0.5 text-[11px] font-semibold text-zinc-500">
              Chef nuevo
            </span>
          )}
        </div>
...
// Modal de detalle del chef — usado por la sección de home y por /chefs.
// Contiene su propia animación de entrada (no depende de un <style> del padre).
export function ChefDetailDialog({
  chef,
  onClose,
}: {
  chef: ChefCard | null;
  ...
  return (
    <>
      ...
      `}</style>
      <Dialog open={!!chef} onOpenChange={(o) => { if (!o) onClose(); }}>
        {chef && (
          <DialogContent className="chef-detail-panel max-h-[90vh] max-w-3xl overflow-y-auto p-0 motion-safe:animate-[chefPanelIn_0.3s_ease-out] md:max-h-none md:overflow-hidden">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2">
              ...
              <div className="flex flex-col p-6 md:p-7">
                {locationLabel(chef.city, chef.country) && (
                ...
                <div className="mt-3">
                  <RatingLine rating={chef.rating} size="md" />
                </div>
    ...
    <section id="chefs" ref={sectionRef} className="relative overflow-hidden bg-white py-28 scroll-mt-20">
      ...
      <div className="container relative z-10 mx-auto max-w-[1280px] px-6">
        ...
        {/* Cards: carrusel horizontal con scroll-snap en mobile (corta el scroll
            vertical largo), grilla de 3/4 en desktop según ancho. El cambio es
            solo de layout vía breakpoints md:/lg: — el diseño de cada card no se toca. */}
        <div
          role="region"
        ...
        >
          {chefs.map((chef, idx) => (
            ...
            >
              <ChefCardCompact chef={chef} onSelect={setSelected} />
            </div>
          ))}
        </div>
      </div>
