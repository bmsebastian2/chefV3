...
function buildActiveEmail(name: string): string {
  return shell(`
...
function buildMagicLinkEmail(name: string, magicLink: string, tempPassword?: string): string {
...
export async function sendClientEmails(opts: {
  ...
  if (!opts.isNewUser) {
    ...
    await resend.emails
      .send({
        ...
        html:    buildActiveEmail(opts.name),
      })
  ...
  const { error } = await resend.emails.send({
    ...
    html:    buildMagicLinkEmail(opts.name, opts.confirmationLink, opts.tempPassword),
  })
